    ...
    def getresults(self, sourcecity, destinationcity, journeydate, trainclass='3A', flightclass='economy', numberofadults=1):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            ...
            if sourcenear == destinationnear:
                ...
                logger.warning("Nearest airports sourceNearest [%s] and destinationNearest [%s] are same as given cities source [%s] and destination [%s]", sourcenear, destinationnear, source, destination)
                return {"flight": []}
