...
#fileHandler = logging.FileHandler('D:/DBSCRIPT_' + today +'.log')
...
#jsonfileHandler = logging.FileHandler('D:/DBSCRIPTJSON_'+today+'.log')
...
def getTrainFare(sourceStation, destinationStation, trainNumber, srcStationInformation, destStationInformation):
    ...
    try:
        jsonResponseTrainFare = urllib.urlopen(
            "http://api.railwayapi.com/fare/train/" + trainNumber + "/source/" + sourceStation + "/dest/" + destinationStation + "/age/20/quota/GN/doj/" + '11-06' + "/apikey/" + trainConstants.ERAILWAYAPI_APIKEY + "/").read()
    except:
        logger.error(
            "Connection Error Getting Fare Data for TrainNumber[%s] ,SourceStation[%s],DestinationStation[%s] ",
            trainNumber, sourceStation, destinationStation)
        return
    fareData = parseAndReturnFare(jsonResponseTrainFare)
    if not fareData:
        logger.error("Response Error Getting Fare Data for TrainNumber[%s] ,SourceStation[%s],DestinationStation[%s] ",
                     trainNumber, sourceStation, destinationStation)
        logger.info("Adding route relation without fare for TrainNumber[%s]", trainNumber)
        routeInformationToCommit = consolidateRouteDataToUpdate(srcStationInformation, destStationInformation,
                                                                trainNumber)
        try:
            models.addStationToRouteMapping(routeInformationToCommit)
        except:
            logger.info(
                "DB Error for TrainNumber[%s] ,SourceStation[%s],DestinationStation[%s]. Failed to commit route information",
                trainNumber, sourceStation, destinationStation)
        return
    else:
        logger.info("Fare Data Fetch Success for TrainNumber[%s] ,SourceStation[%s],DestinationStation[%s]",
                    trainNumber, sourceStation, destinationStation)
    finalInformationToCommit = consolidateRelationDatatoUpdate(srcStationInformation, destStationInformation, fareData,
                                                               trainNumber)
    ...
    try:
        ...
        logger.info(
            "DB Error for TrainNumber[%s] ,SourceStation[%s],DestinationStation[%s]. Failed to commit fare information",
            trainNumber, sourceStation, destinationStation)
...
def main():
    ...
    for line in lines:
        ...
        try:
            jsonResponseTrainRoute = urllib.urlopen(
                "http://api.railwayapi.com/route/train/" + trainNumber + "/apikey/" + trainConstants.ERAILWAYAPI_APIKEY + "/").read()
        except:
        ...
        if len(routeStations) == 0:
            ...
            logger.info("Route Data Fetch Success for TrainNumber[%s], No of Routes[%s].", trainNumber,
                        len(routeStations))
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            ...
            while index < numberOfStations - 1:
                ...
                while iterator <= numberOfStations - 1:
                    executor.submit(
                        getTrainFare(routeStations[index]["code"], routeStations[iterator]["code"], trainNumber,
                                     routeStations[index], routeStations[iterator]))
                    iterator += 1
                index += 1
